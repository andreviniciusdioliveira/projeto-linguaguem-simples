...
def processar():
    ...
    try:
        ...
        # LIMPAR PRÓXIMOS PASSOS DO TEXTO ANTES DE ENVIAR
        texto_simplificado = limpar_proximos_passos(texto_simplificado)
...
def limpar_proximos_passos(texto):
    """Remove completamente a seção PRÓXIMOS PASSOS do texto"""
    import re
    
    # Remover a seção PRÓXIMOS PASSOS e todo seu conteúdo até a próxima seção
    patterns = [
        r'(?:^|\n)(?:###\s*)?(?:📅\s*)?PRÓXIMOS PASSOS.*?(?=\n(?:###\s*)?(?:📊|📑|⚖️|💰|⚠️|💡|📚)|$)',
        r'(?:^|\n)(?:###\s*)?(?:📅\s*)?Próximos Passos.*?(?=\n(?:###\s*)?(?:📊|📑|⚖️|💰|⚠️|💡|📚)|$)',
        r'(?:^|\n)(?:###\s*)?(?:📅\s*)?PROXIMOS PASSOS.*?(?=\n(?:###\s*)?(?:📊|📑|⚖️|💰|⚠️|💡|📚)|$)',
    ...
    for pattern in patterns:
        texto = re.sub(pattern, '', texto, flags=re.IGNORECASE | re.DOTALL | re.MULTILINE)
    
    # Remover linhas individuais que sejam típicas de próximos passos
    linhas_remover = [
        r'^\s*[•▸\-]\s*Aguardar.*
...
if __name__ == "__main__":
    port = int(os.getenv("PORT", 8080))
    app.run(host="0.0.0.0", port=port, debug=False),
        r'^\s*[•▸\-]\s*Apresentar contrarrazões.*
